-- ============================================================
-- Verification disclosure logical identity hardening
-- Preserves historical rows while enforcing canonical idempotency.
-- ============================================================

BEGIN;

ALTER TABLE public.verification_disclosures
  ADD COLUMN IF NOT EXISTS is_canonical BOOLEAN;

ALTER TABLE public.verification_disclosures
  ADD COLUMN IF NOT EXISTS superseded_by_disclosure_id UUID;

ALTER TABLE public.verification_disclosures
  ALTER COLUMN is_canonical SET DEFAULT true;

UPDATE public.verification_disclosures
...
UPDATE public.verification_disclosures vd
SET
  is_canonical = (vd.id = ranked.canonical_id),
  superseded_by_disclosure_id = CASE
    WHEN vd.id = ranked.canonical_id THEN NULL
    ELSE ranked.canonical_id
  END
FROM ranked
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'verification_disclosures_superseded_by_fkey'
      AND conrelid = 'public.verification_disclosures'::regclass
  ) THEN
    ALTER TABLE public.verification_disclosures
      ADD CONSTRAINT verification_disclosures_superseded_by_fkey
      FOREIGN KEY (superseded_by_disclosure_id)
      REFERENCES public.verification_disclosures(id)
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'verification_disclosures_canonical_reference_state'
      AND conrelid = 'public.verification_disclosures'::regclass
  ) THEN
    ALTER TABLE public.verification_disclosures
      ADD CONSTRAINT verification_disclosures_canonical_reference_state
      CHECK (
        (is_canonical AND superseded_by_disclosure_id IS NULL)
        OR (NOT is_canonical AND superseded_by_disclosure_id IS NOT NULL)
      )
      NOT VALID;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'verification_disclosures_no_self_supersede'
      AND conrelid = 'public.verification_disclosures'::regclass
  ) THEN
    ALTER TABLE public.verification_disclosures
      ADD CONSTRAINT verification_disclosures_no_self_supersede
      CHECK (
        superseded_by_disclosure_id IS NULL
        OR superseded_by_disclosure_id <> id
      )
      NOT VALID;
  END IF;
END;
...
ALTER TABLE public.verification_disclosures
  VALIDATE CONSTRAINT verification_disclosures_superseded_by_fkey;

ALTER TABLE public.verification_disclosures
  VALIDATE CONSTRAINT verification_disclosures_canonical_reference_state;

ALTER TABLE public.verification_disclosures
  VALIDATE CONSTRAINT verification_disclosures_no_self_supersede;

CREATE UNIQUE INDEX IF NOT EXISTS idx_verification_disclosures_onboarding_canonical_identity
  ON public.verification_disclosures (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_verification_disclosures_upgrade_canonical_identity
  ON public.verification_disclosures (
    merchant_id,
    user_id,
    plan_type,
...
BEGIN
  ...
  IF p_onboarding_session_id IS NOT NULL THEN
    PERFORM pg_advisory_xact_lock(
      hashtextextended(
        'verification-disclosure:onboarding:' ||
        p_onboarding_session_id::text || ':' ||
        v_plan_type || ':' ||
        v_context || ':' ||
  ...
  ELSE
    PERFORM pg_advisory_xact_lock(
      hashtextextended(
        'verification-disclosure:upgrade:' ||
