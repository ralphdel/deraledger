-- READ ONLY. Paste directly into Supabase SQL Editor after migration 019.
-- Every schema/security contract row must PASS. Compare the final row-count JSON
-- byte-for-byte with the preflight preservation.row_counts value.
...
expected_constraints(table_name, constraint_name, constraint_type) AS (
  VALUES
    ('invoices', 'valid_invoice_type', 'c'),
    ('roles', 'roles_merchant_id_fkey', 'f'),
...
column_checks AS (
  SELECT
    ...
    CASE
      WHEN attribute.attname IS NOT NULL
       AND format_type(attribute.atttypid, attribute.atttypmod) = expected.expected_type
      THEN 'PASS' ELSE 'FAIL'
    END AS status,
    CASE
      WHEN attribute.attname IS NULL THEN 'Required column is missing.'
      WHEN format_type(attribute.atttypid, attribute.atttypmod) <> expected.expected_type THEN 'Existing type is incompatible.'
...
index_checks AS (
  SELECT
    ...
    CASE WHEN index_row.tablename = expected.table_name THEN 'PASS' ELSE 'FAIL' END AS status,
    CASE WHEN index_row.indexname IS NULL THEN 'Required index is missing.' ELSE 'Index is present.' END AS details
...
constraint_checks AS (
  SELECT
    'constraint.public.' || expected.table_name || '.' || expected.constraint_name AS check_name,
    'constraint'::text AS object_type,
    'type=' || expected.constraint_type || ' validated=true' AS expected,
    CASE
      WHEN constraint_row.oid IS NULL THEN 'missing'
      ELSE format('type=%s validated=%s', constraint_row.contype, constraint_row.convalidated)
    END AS actual,
    CASE
      WHEN constraint_row.contype::text = expected.constraint_type
       AND constraint_row.convalidated
      THEN 'PASS' ELSE 'FAIL'
    END AS status,
    'Required identity, ownership, or invoice contract constraint.'::text AS details
  FROM expected_constraints expected
  LEFT JOIN pg_constraint constraint_row
    ON constraint_row.conrelid = to_regclass(format('public.%I', expected.table_name))
...
grant_checks AS (
  SELECT
    ...
    CASE
      ...
      THEN 'PASS' ELSE 'FAIL'
    END AS status,
    'Server actions retain service-role writes; no payment/provider grant is changed.'::text AS details
  FROM table_contract contract
)
...
SELECT
  ...
  'Exactly equal to preflight preservation.row_counts' AS expected,
  jsonb_build_object(
  ...
  'WARN' AS status,
  'SQL cannot remember a prior SQL Editor session without writing state; compare this JSON to preflight manually, then treat it as PASS.' AS details
