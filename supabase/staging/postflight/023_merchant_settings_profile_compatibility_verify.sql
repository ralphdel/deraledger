-- Read-only SQL Editor postflight for Migration 023.
...
WITH expected_columns(
  ...
  SELECT
    ...
    'Migration 023 must preserve the existing merchants table.'::text AS details
  FROM relation_state
  ...
  SELECT
    ...
    format(
      ...
      formatted_type,
      CASE
        WHEN added_by_023 AND allow_not_null THEN 'nullable or historical NOT NULL with a safe default'
        WHEN expected_not_null THEN 'not null'
        ELSE 'nullable'
      END,
      CASE WHEN added_by_023 THEN '; Migration 023 compatible defaults only' ELSE '' END
    ),
    CASE
    ...
    CASE
      ...
      WHEN added_by_023 AND actual_not_null AND NOT allow_not_null THEN 'FAIL'
      WHEN added_by_023 AND actual_not_null AND actual_default IS NULL THEN 'FAIL'
      WHEN added_by_023 AND NOT (COALESCE(actual_default, '<NULL>') = ANY (allowed_defaults)) THEN 'FAIL'
      WHEN NOT added_by_023 AND actual_not_null <> expected_not_null THEN 'FAIL'
      ELSE 'PASS'
    END,
    CASE
      WHEN actual_type IS NULL THEN 'The Settings/Profile/Verification runtime contract is incomplete.'
      WHEN actual_type <> formatted_type THEN 'The existing column type is incompatible with runtime.'
      WHEN added_by_023 AND actual_not_null AND NOT allow_not_null THEN 'Migration 023 profile additions must remain nullable.'
      WHEN added_by_023 AND actual_not_null AND actual_default IS NULL THEN 'A NOT NULL field without a safe default could break merchant creation.'
      WHEN added_by_023 AND NOT (COALESCE(actual_default, '<NULL>') = ANY (allowed_defaults)) THEN 'The existing default is outside the canonical fail-closed profile contract.'
      WHEN NOT added_by_023 AND actual_not_null <> expected_not_null THEN 'The clean-production prerequisite has unexpected nullability.'
      ELSE 'The runtime column contract is satisfied.'
    END
  FROM column_state
  ...
  SELECT
    ...
    'PASS',
    'Migration 023 does not change existing RLS policies or grants.'
  FROM relation_state
)
