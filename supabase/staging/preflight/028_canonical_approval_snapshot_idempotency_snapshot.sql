WITH required_columns AS (
  ...
  UNION ALL
  SELECT 'prerequisite.workspace_linkage',CASE WHEN merchant_id_is_unique AND workspace_id_is_unique THEN 'PASS' ELSE 'FAIL' END,
    'Merchant/workspace primary identities are unique; M028 uses an exact count-one cross-link and fails closed for absent or ambiguous row linkage' FROM workspace_key_shape
  UNION ALL
