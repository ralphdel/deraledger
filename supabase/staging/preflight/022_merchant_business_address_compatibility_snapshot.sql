-- Read-only SQL Editor preflight for Migration 022.
BEGIN;
...
WITH expected(column_name) AS (
  ...
  SELECT
    ...
    'Migration 022 requires the existing merchants table and does not replace it.'::text AS details
  FROM relation_state
  ...
  SELECT
    ...
    END,
    CASE
      WHEN actual_type IS NULL THEN 'Migration 022 will add this nullable text column without backfilling existing rows.'
      WHEN actual_type = 'text' AND NOT actual_not_null AND actual_default IS NULL THEN 'The existing column is compatible; the migration is idempotent.'
      ELSE 'Migration 022 does not rewrite an incompatible existing column.'
    END
  FROM column_state
  ...
  SELECT
    ...
    'PASS',
    'Adding nullable columns does not alter existing RLS policies or table grants.'
  FROM security_state
)
...
-- Stop on every FAIL. Missing canonical address columns are expected WARN rows.
ROLLBACK;
