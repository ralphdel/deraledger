...
assert.match(preflightSql, /workspace_key_shape[\s\S]*merchant_id_is_unique[\s\S]*workspace_id_is_unique/);
assert.match(preflightSql, /prerequisite\.workspace_linkage[\s\S]*count-one cross-link/);
