...
assert.match(migration, /PERFORM 1 FROM public\.merchants m WHERE m\.id = p_merchant_id;/);
assert.doesNotMatch(migration, /PERFORM 1 FROM public\.merchants m WHERE m\.id = p_merchant_id FOR KEY SHARE/);
...
assert.match(rehearsalScript, /LOCAL_M029_REHEARSAL_KEYWORD_CONNECTION_STRING_REQUIRED/);
assert.match(rehearsalScript, /LOCAL_M029_REHEARSAL_PORT_REQUIRED/);
assert.match(rehearsalScript, /LOCAL_M029_REHEARSAL_LOCAL_PORT_REQUIRED/);
assert.match(rehearsalScript, /LOCAL_M029_REHEARSAL_USER_REQUIRED/);
...
assert.match(rehearsalScript, /function Get-LocalPgKeywordConnectionValue/);
assert.match(rehearsalScript, /Get-LocalPgKeywordConnectionValue -ConnectionString \$ConnectionString -Name 'host'/);
assert.match(rehearsalScript, /Get-LocalPgKeywordConnectionValue -ConnectionString \$ConnectionString -Name 'port'/);
assert.match(rehearsalScript, /Get-LocalPgKeywordConnectionValue -ConnectionString \$ConnectionString -Name 'dbname'/);
assert.match(rehearsalScript, /Get-LocalPgKeywordConnectionValue -ConnectionString \$ConnectionString -Name 'user'/);
...
assert.match(rehearsalScript, /'-h', \$Target\.Host, '-p', \$Target\.Port,[\s\S]*'-U', \$Target\.User, '-d', \$Target\.Database, '-f', \$FilePath/);
assert.doesNotMatch(rehearsalScript, /'-d', \$LocalConnectionString(?:\.Trim\(\))?/);
...
assert.doesNotMatch(rehearsalScript, /&\s+\$env:ComSpec\s+\/d\s+\/c/);
assert.doesNotMatch(rehearsalScript, /&\s+cmd\.exe\s+\/d\s+\/c/i);
...
assert.match(rehearsalScript, /GRANT SELECT ON TABLE auth\.users, public\.merchants, public\.workspaces, public\.solo_plus_cases TO service_role/);
assert.match(rehearsalScript, /CREATE TEMP TABLE canonical_workspace_scenario_diagnostics/);
assert.match(rehearsalScript, /GET STACKED DIAGNOSTICS\s+v_sqlstate = RETURNED_SQLSTATE,\s+v_sql_message = MESSAGE_TEXT/);
assert.match(rehearsalScript, /canonical_workspace_link_invocation_failed[\s\S]*INSERT INTO pg_temp\.canonical_workspace_scenario_diagnostics/);
assert.match(rehearsalScript, /service_role_prerequisite_reads[\s\S]*PERFORM 1 FROM auth\.users[\s\S]*PERFORM 1 FROM public\.merchants[\s\S]*PERFORM 1 FROM public\.workspaces/);
assert.match(rehearsalScript, /local_sqlstate[\s\S]*local_sql_message/);
assert.doesNotMatch(migration, /GET STACKED DIAGNOSTICS|canonical_workspace_scenario_diagnostics/);
