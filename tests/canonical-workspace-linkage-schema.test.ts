...
assert.match(postflight, /has_function_privilege\('PUBLIC','public\.reconcile_canonical_merchant_workspace_link_v1\(uuid,uuid,text\)','EXECUTE'\)/);
assert.match(postflight, /NOT has_table_privilege\('anon','public\.merchant_canonical_workspaces'/);
assert.match(postflight, /NOT has_table_privilege\('authenticated','public\.merchant_canonical_workspaces'/);
