...
assert.doesNotMatch(rehearsalScript, /LocalConnectionString|Get-LocalPgConnectionParts|Get-LocalPgKeywordConnectionValue/);
assert.match(rehearsalScript, /\[string\]\$LocalHost = '127\.0\.0\.1'/);
assert.match(rehearsalScript, /\[int\]\$LocalPort = 55432/);
assert.match(rehearsalScript, /\[string\]\$LocalUser = 'postgres'/);
assert.match(rehearsalScript, /\[Parameter\(Mandatory = \$true\)\]\[string\]\$LocalDatabase/);
assert.match(rehearsalScript, /Assert-LocalDisposableTarget -HostName \$LocalHost -Port \$LocalPort -DatabaseName \$LocalDatabase -UserName \$LocalUser/);
assert.match(rehearsalScript, /\$target\.Host -notin @\('localhost', '127\.0\.0\.1'\)/);
assert.match(rehearsalScript, /\$target\.Port -ne 55432/);
assert.match(rehearsalScript, /Read-Host "Enter LOCAL PostgreSQL password for \$\(\$Target\.User\)@\$\(\$Target\.Host\)" -AsSecureString/);
assert.match(rehearsalScript, /\$env:PGPASSWORD = \[Runtime\.InteropServices\.Marshal\]::PtrToStringBSTR\(\$LocalPasswordBstr\)/);
assert.match(rehearsalScript, /\$env:PGPASSWORD = \$OriginalPGPASSWORD/);
...
assert.doesNotMatch(rehearsalScript, /-d\s+\$LocalConnectionString|postgres(?:ql)?:\/\//i);
