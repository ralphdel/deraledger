...
function Get-Sha256Hex {
  ...
  try { return ([System.BitConverter]::ToString($Sha256.ComputeHash([System.Text.Encoding]::UTF8.GetBytes($Value)))).Replace('-', '').ToLowerInvariant() }
  finally { if ($null -ne $Sha256) { $Sha256.Dispose() } }
}
function Read-StagingTarget {
  $DbHost=(Read-Host 'Exact reviewed staging Supabase pooler host').Trim().ToLowerInvariant(); $DbPort=(Read-Host 'Staging pooler port (default 5432)').Trim(); $DbName=(Read-Host 'Staging database name').Trim().ToLowerInvariant(); $DbUser=(Read-Host 'Staging pooler database user').Trim().ToLowerInvariant(); $ConfirmedStagingProjectRef=(Read-Host 'Confirm the reviewed staging Supabase project ref').Trim().ToLowerInvariant()
  ...
  if($DbHost -in @('localhost','127.0.0.1','::1')){throw 'ADMIN_READINESS_STAGING_LOCALHOST_REJECTED'};if($DbHost -notmatch '^[a-z0-9-]+\.pooler\.supabase\.com$'){throw 'ADMIN_READINESS_STAGING_UNRECOGNIZED_HOST_REJECTED'};$UserMatch=[regex]::Match($DbUser,'^postgres\.([a-z0-9]+)$');if(-not $UserMatch.Success){throw 'ADMIN_READINESS_STAGING_UNRECOGNIZED_USER_REJECTED'};$ParsedStagingProjectRef=$UserMatch.Groups[1].Value;if($ApprovedStagingUser -cne "postgres.$ApprovedStagingProjectRef"){throw 'ADMIN_READINESS_STAGING_APPROVED_IDENTITY_MANIFEST_INVALID'};if($DbHost -cne $ApprovedStagingPoolerHost){throw 'ADMIN_READINESS_STAGING_POOLER_HOST_NOT_APPROVED'};if($DbPort -cne $ApprovedStagingPort){throw 'ADMIN_READINESS_STAGING_PORT_NOT_APPROVED'};if($DbName -cne $ApprovedStagingDatabase){throw 'ADMIN_READINESS_STAGING_DATABASE_NOT_APPROVED'};if($DbUser -cne $ApprovedStagingUser){throw 'ADMIN_READINESS_STAGING_USER_NOT_APPROVED'};if($ParsedStagingProjectRef -cne $ApprovedStagingProjectRef){throw 'ADMIN_READINESS_STAGING_PROJECT_REF_NOT_APPROVED'};if($ConfirmedStagingProjectRef -cne $ApprovedStagingProjectRef){throw 'ADMIN_READINESS_STAGING_PROJECT_REF_CONFIRMATION_MISMATCH'};if($DbHost -match '(?i)(prod|production|live)' -or $DbName -match '(?i)(prod|production|live)' -or $DbUser -match '(?i)(prod|production|live)'){throw 'ADMIN_READINESS_STAGING_PRODUCTION_INDICATOR_REJECTED'};if($DbPort -notmatch '^[0-9]{1,5}$' -or [int]$DbPort -lt 1 -or [int]$DbPort -gt 65535){throw 'ADMIN_READINESS_STAGING_PORT_REJECTED'};[pscustomobject]@{DbHost=$DbHost;DbPort=$DbPort;DbName=$DbName;DbUser=$DbUser;ApprovedStagingProjectRef=$ApprovedStagingProjectRef;ApprovedStagingPoolerHost=$ApprovedStagingPoolerHost;TargetFingerprint=(Get-Sha256Hex "$DbHost|$DbPort|$DbName|$DbUser|$ApprovedStagingProjectRef")}
}
