...
const reference = `SUB-RECOVERY-PROOF-${crypto.randomBytes(4).toString("hex").toUpperCase()}`;
const email = `proof-recovery-${stamp}@deraledger.app`;
const businessName = `Proof Recovery ${stamp}`;
...
const response = await fetch(`${monnifyBase}/api/v1/merchant/transactions/init-transaction`, {
  ...
  body: JSON.stringify({
    ...
    paymentDescription: "DeraLedger proof recovery subscription payment",
