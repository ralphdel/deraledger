...
If that session-temp row is absent, local-GUC-gated branch notices identify the exact source-lookup return or exception route without changing the RPC result. The Lite/Business count-one query now aliases every `merchant_compliance_reviews` column, including `review_source.profile_id`, to avoid collision with the function's `RETURNS TABLE` output identifiers. When the local diagnostic GUC is enabled and the review-source branch throws, `GET STACKED DIAGNOSTICS` prints SQLSTATE, message, detail, hint, and context as disposable-harness-only `psql` notices. Those values are never added to the RPC result payload or enabled outside the local rehearsal. This remains a disposable-only diagnostic requirement before any staging consideration.
