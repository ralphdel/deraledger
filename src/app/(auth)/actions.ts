...
export async function forgotPasswordAction(email: string) {
  ...
  const verifyLink = `${appUrl}/auth/verify?token=${otp}&email=${encodeURIComponent(email)}&type=recovery&next=${encodeURIComponent('/reset-password')}`;

  // Send the branded email via Brevo
  const emailResult = await sendPasswordResetEmail(email, verifyLink);
...
export async function resendActivationLinkAction(email: string) {
  ...
  const setPasswordLink = `${appUrl}/auth/verify?token=${otp}&email=${encodeURIComponent(email.toLowerCase().trim())}&type=magiclink&next=${encodeURIComponent('/onboarding/set-password')}`;
