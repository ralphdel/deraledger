...
import { parseSoloPlusAdminCasesQuery } from "@/lib/solo-plus/server/route-contracts";
...
type SoloPlusAdminCasesRouteDependencies = {
...
export function createSoloPlusAdminCasesRouteHandler(
  dependencies: SoloPlusAdminCasesRouteDependencies,
) {
  return async function GET(request: Request): Promise<NextResponse> {
    ...
    let query;
    try {
      query = parseSoloPlusAdminCasesQuery(request);
    } catch (error) {
      return buildErrorResponse(
        400,
    ...
    try {
      ...
      return buildErrorResponse(
        500,
        "Solo Plus admin case queue failed unexpectedly.",
        "INTERNAL_ERROR",
      );
    }
  };
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const handler = createSoloPlusAdminCasesRouteHandler({
    ...
    onUnexpectedError: (error) => {
      console.error("Solo Plus admin cases route failed:", error);
    },
  });
