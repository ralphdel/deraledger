...
import { 
  ...
  Coins, Landmark as TreasuryIcon, AlertTriangle
} from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

// Global administrative requests seed matching database model (Section 12.1)
const MOCK_ADMIN_REFUND_REQUESTS = [
  {
    id: "ref-9012-n28",
  ...
  },
  {
    ...
    refund_type: "PARTIAL",
    amount: 120000,
    status: "OFFSET_APPLIED", // Offset Applied status
    risk_score: 40,
    requires_manual_review: true,
    sla_time: "SLA Overdue (24h overdue)",
  ...
  },
  {
    ...
    risk_score: 75, // High value stablecoin risk
    requires_manual_review: true,
    sla_time: "SLA ALERT (1h remaining)",
    created_at: "2026-05-20T11:00:00Z"
  },
  {
    id: "ref-6671-k99",
    reference: "REF_9028104",
    ...
    risk_score: 85, // Velocity risk matched
    requires_manual_review: true,
    sla_time: "SLA OK (22h remaining)",
...
export default function AdminRefundsQueue() {
  ...
  const filteredRequests = useMemo(() => {
    return MOCK_ADMIN_REFUND_REQUESTS.filter(r => {
  ...
  const metrics = useMemo(() => {
    const total = MOCK_ADMIN_REFUND_REQUESTS.length;
    const pendingReview = MOCK_ADMIN_REFUND_REQUESTS.filter(r => r.status === "REQUESTED" || r.status === "REVIEWING").length;
    const cryptoCount = MOCK_ADMIN_REFUND_REQUESTS.filter(r => r.payment_rail === "BREET_CRYPTO").length;
    const offsetsCount = MOCK_ADMIN_REFUND_REQUESTS.filter(r => r.status === "OFFSET_APPLIED").length;
    const totalExposure = MOCK_ADMIN_REFUND_REQUESTS.reduce((sum, r) => sum + r.amount, 0);
                ...
                {filteredRequests.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-6 py-12 text-center text-neutral-400">
                      No global refund requests match current filters.
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <tr key={req.id} className="hover:bg-neutral-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 space-y-0.5">
                          ...
                          <div className="text-xs text-neutral-400">{req.customer_email}</div>
                        </td>
                        <td className="px-6 py-4">
